...
function RoundSetup({onConfirm, onBack}) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12 bg-bg">
      <div className="w-full max-w-md">
        <button onClick={onBack} className="text-muted hover:text-text transition text-sm mb-8">
          ← Genres
        </button>

        <h1 className="font-display text-4xl font-semibold text-text text-center mb-10">
          Set up your quiz
        </h1>
          ...
          <div className="flex flex-col gap-3">
            {DIFFICULTIES.map(d => (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-lg bg-surface text-left ring-2 transition
                  ${difficulty === d.id ? d.ring : "ring-transparent"}`}
              >
                <span className={`w-3 h-3 rounded-full shrink-0 ${d.color}`} />
                <span>
                  <span className="font-display text-text text-lg block">{d.label}</span>
        ...
        <button
          onClick={() => onConfirm({ rounds, difficulty })}
          className="w-full bg-accent text-text font-display text-lg py-3 rounded-lg hover:opacity-90 transition">
          Start quiz
        </button>
      </div>
    </div>
